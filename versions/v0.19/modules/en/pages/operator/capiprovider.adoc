...
:description: Install CAPI providers to manage Cluster API Operator manifests declaratively using GitOps to enable provisioning and configuration of providers, such as AWS
:revdate: 2026-06-26
...
Every field provided by the upstream CAPI Operator resource for the desired `spec.type` is also available in the spec of the `CAPIProvider` resource. Feel free to refer to upstream configuration https://cluster-api-operator.sigs.k8s.io/topics/configuration/[guides] for advanced scenarios.
