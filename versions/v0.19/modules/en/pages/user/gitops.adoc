...
- **Use GitOps Integration Tools When Available:**
  When configuring a GitOps setup, take into consideration available tooling and features it provides. An example of this is https://rancher-sandbox.github.io/cluster-api-addon-provider-fleet/00_intro.html[`CAAPF`], which automates the Fleet GitOps installation on `CAPI` clusters and provides convenient automation.
