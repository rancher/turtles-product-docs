...
This section provides best practices for implementing GitOps with Turtles and Cluster API (CAPI), focusing on using https://documentation.suse.com/cloudnative/continuous-delivery/latest/en/index.html[Continuous Delivery] in combination with the *Cluster API Addon Provider Fleet*(`CAAPF`) for automating agent provisioning and integration into the `CAPI` ecosystem.
endif::[]
