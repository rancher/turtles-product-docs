...
* The CAPRKE2 and CAAPF providers must be installed as a prerequisite for using all ClusterClasses excluding Azure AKS.
** CAPRKE2 (Bootstrap and Control Plane) provider installation:
+
.Click to Expand
[%collapsible]
======
...
* https://github.com/rancher/cluster-api-provider-rke2[Bootstrap/Control Plane provider for RKE2] or https://github.com/kubernetes-sigs/cluster-api[Bootstrap/Control Plane provider for Kubeadm], example of Kubeadm installation:
+
[source,yaml]
----
apiVersion: v1
...
. Upgrade {product_name} to `v0.24` or higher.  
...
metadata:
  ...
  labels:
    cni: calico
  annotations:
    cluster-api.cattle.io/cluster-description: "Modified cluster.mgmt.cattle.io object"
----
