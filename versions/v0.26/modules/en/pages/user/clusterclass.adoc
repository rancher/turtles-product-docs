...
+
--
To prepare the management Cluster, we are going to install the https://cluster-api-gcp.sigs.k8s.io/[Cluster API Provider GCP].
...
GCP credentials are provided per-cluster using `credentialsRef` on `GCPCluster` and `GCPManagedCluster` resources, rather than as a global controller variable. The credential Secret is created alongside each workload Cluster (see the <<_creating_a_cluster,Creating a Cluster>> section below).
...
Note that some variables are left to the user to substitute. +
+
First, create a Secret containing the GCP Service Account credentials in the same namespace where the Cluster will be created. The Secret must have a `credentials` key containing the raw Service Account JSON:
+
[source,yaml]
----
apiVersion: v1
kind: Secret
metadata:
  name: gcp-quickstart-credentials
  namespace: default
type: Opaque
stringData:
  credentials: |
...
----
+
Then create the Cluster, referencing the credential Secret via the `credentialSecretName` and `credentialSecretNamespace` variables:
+
[source,yaml]
----
apiVersion: cluster.x-k8s.io/v1beta2
  ...
  topology:
    ...
    variables:
      ...
      - name: credentialSecretName
        value: gcp-quickstart-credentials
