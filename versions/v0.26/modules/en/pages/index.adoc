= Overview
:description: Understand the architectural features and core concepts of SUSE Rancher Prime Cluster API, including integration with Rancher Manager and Cluster API
...
[CAUTION]
====
Cluster API Add-on Provider for Fleet (CAAPF) is deprecated as of Rancher v2.14.1 and is no longer installed by default. We encourage you to move away from CAAPF in preparation for its eventual removal (following Rancher deprecation policies). You can refer to xref:./user/applications.adoc[this guide] for more information on how to replace CAAPF with other alternatives.

If you wish to continue using it, refer to the xref:user/caapf.adoc[CAAPF user guide] for more information.
====
