...
Dieser Abschnitt bietet Best Practices für die Implementierung von GitOps mit Turtles und der Cluster API (CAPI) und konzentriert sich auf die Verwendung von https://documentation.suse.com/cloudnative/continuous-delivery/latest/en/index.html[Continuous Delivery] in Kombination mit dem *Cluster API Addon Provider Fleet*(`CAAPF`) zur Automatisierung der Agentenbereitstellung und Integration in das `CAPI` Ökosystem.
endif::[]
...
- **Addon-Konfigurationen isolieren:**
  Speichern Sie Anwendungsmanifeste in `applications/`, um sicherzustellen, dass sie unabhängig von `Clusters` und `ClusterClasses` sind. Dies ermöglicht eine selektive oder gruppierte Bereitstellung basierend auf der Benutzeranforderung oder übereinstimmenden Selektoren und verhindert Störungen der Clusterinfrastruktur. Bitte beachten Sie, dass Helm-Chart-Abhängigkeiten vor der Bereitstellung von Addons https://documentation.suse.com/cloudnative/continuous-delivery/latest/en/explanations/gitrepo-content.html#_fleet_yaml[aktualisiert] werden müssen.
- **Verschachtelte GitRepo-Ressourcen nutzen:**
  Nutzen Sie https://documentation.suse.com/cloudnative/continuous-delivery/latest/en/explanations/gitrepo-content.html#_nested_gitrepo_crs[verschachtelte] `GitRepo` Ressourcen in Fleet, um den Inhalt des Repositories effizient zu strukturieren. Dieser Ansatz ermöglicht eine modulare Bereitstellung, die `ClusterClass` Definitionen, erforderliche Addons/Anwendungen und Clusterkonfigurationen kombiniert und dabei die Trennung der Anliegen beibehält.
...
- **Nutzen Sie Vergleichspatches:**
  Fleet ermöglicht das selektive Ignorieren von Ressourcenunterschieden, wenn das Bundle die richtigen Regeln für die https://documentation.suse.com/cloudnative/continuous-delivery/latest/en/troubleshooting.html#_suse_rancher_prime_continuous_delivery_deployment_stuck_in_modified_state[`diff` Erkennung] enthält. Dies würde verhindern, dass Bundles in einem modifizierten Zustand stecken bleiben.
endif::[]
ifeval::["{build-type}" == "community"]
- **Getrennte CAPI-Cluster und ClusterClass-Konfigurationen:**
  Halten Sie die CAPI-Clusterkonfigurationen (`clusters/`) von den CAPI `ClusterClass`-Definitionen (`templates/`) getrennt. Diese Trennung ermöglicht es, mehrere Cluster aus gemeinsamen Vorlagen bereitzustellen, während das Risiko unbeabsichtigter Infrastrukturänderungen verringert wird.
