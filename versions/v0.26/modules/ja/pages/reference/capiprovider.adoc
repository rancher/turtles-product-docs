...
アップストリームのCAPI Operator `CAPIProviderSpec`リソースによって提供されるすべてのフィールドは、`CAPIProvider`リソースのspecにも利用可能です。高度なシナリオについては、アップストリームの設定link:https://cluster-api-operator.sigs.k8s.io/topics/configuration/provider-spec-configuration[ガイド]を参照してください。
