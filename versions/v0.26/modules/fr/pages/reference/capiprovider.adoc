...
Chaque champ fourni par la ressource CAPI Operator `CAPIProviderSpec` en amont est également disponible dans la spécification de la ressource `CAPIProvider`. N'hésitez pas à vous référer à la configuration en amont link:https://cluster-api-operator.sigs.k8s.io/topics/configuration/provider-spec-configuration[guides] pour des scénarios avancés.
