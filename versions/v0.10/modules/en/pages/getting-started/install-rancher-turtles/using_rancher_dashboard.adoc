...
[CAUTION]
====
In case you need to review the list of prerequisites (including `cert-manager`), you can refer to link:./intro.adoc#_prerequisites[this table].
====


[IMPORTANT]
====
Before link:./using_helm.adoc#install-rancher-turtles-with-cluster-api-operator-as-a-helm-dependency[installing Rancher Turtles] in your Rancher environment, Rancher's `embedded-cluster-api` functionality must be disabled. This includes also cleaning up Rancher-specific webhooks that otherwise would conflict with CAPI ones.
